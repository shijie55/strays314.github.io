            ...
            <div class="box-body pad-v">
                <form name="import_form" id="import_form" class="form-horizontal" method="post" action="" >
                     <div class="form-group">
                        <label for="SYS_TITLE" class="col-sm-2 control-label">系统名称：</label>
                        <div class="col-sm-4 col-xs-12">
                           <input type="text" class="form-control" name="SYS_TITLE" value="<?php echo ($SYS_TITLE); ?>">
                        </div>
                        <div class="pad-t-sm col-sm-6 col-xs-12"></div>
                     </div>
                     <div class="form-group">
                        <label for="SYS_LOGO" class="col-sm-2 col-xs-12 control-label">Logo：</label>
                        <div class="col-sm-3 col-xs-8">
                           <p><input type="text" class="form-control" name="SYS_LOGO" id="SYS_LOGO" value="<?php echo C('SYS_LOGO');?>" /></p>
                           <p><button type="button" class="btn btn-danger upload_button" id="file_upload_01" data-input="SYS_LOGO" data-img="SYS_LOGO_PIC">上传</button><i class="pull-right" id="loading" style="display:none;"><img src="/Public/admin/dist/img/loading.gif" /></i></p>
                           <p><small class="text-muted">建议尺寸：160/160(px)</small></p>                           
                        </div>
                        <div class="col-sm-1 col-xs-4">
                           <img src="<?php echo C('SYS_LOGO');?>" id="SYS_LOGO_PIC" class="img-responsive" />
                           
                        </div>
                     </div>
                     <!--
                     <div class="form-group">
                        <label for="SYS_PUT_PIC" class="col-sm-2 control-label">点位图片要求：</label>
                        <div class="col-sm-4 col-xs-12">
                           <textarea class="form-control" name="SYS_PUT_PIC"><?php echo ($SYS_PUT_PIC); ?></textarea>
                        </div>
                        <div class="pad-t-sm col-sm-6 col-xs-12">多个说明间用"|"隔开</div>
                     </div>
                     -->
...
$(function(){
   $('.btn-sub-fm').click(function(){
	 ...
	 //alert(data);
	 $.ajax({
	    url : "<?php echo U('/Role/updatesysconfig');?>",
		type : "POST",
		data : data,
		success : function(d){
		   var d=$.parseJSON(d);
