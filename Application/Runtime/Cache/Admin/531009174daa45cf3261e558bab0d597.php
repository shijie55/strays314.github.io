        ...
        <div class="col-xs-12">
          <div class="box box-danger">
            <div class="box-header with-border">                
                <a class="btn btn-success btn-sm btn-sub-taxis" href="javascript:void(0);"><i class="fa fa-sort-alpha-desc"></i> 更新排序</a> <a class="btn btn-info btn-sm" href="<?php echo U('/Category/add');?>"><i class="fa fa-plus"></i> 添加栏目</a>
             </div>
            <!-- /.box-header -->
            <div class="box-body">
             ...
             <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                   <?php if(is_array($catelis)): $i = 0; $__LIST__ = $catelis;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rs): $mod = ($i % 2 );++$i;?><tr>
                      <td class="text-center">
                         <input class="text-center" size="4" value="<?php echo ($rs['taxis']); ?>" data-id="<?php echo ($rs['id']); ?>" type="text" name="fortaxis">
                      </td>
                      <td class="text-center text-red"><?php echo ($rs['id']); ?></td>
                      <td>
                         <?php $__FOR_START_3230__=0;$__FOR_END_3230__=$rs['level'];for($i=$__FOR_START_3230__;$i < $__FOR_END_3230__;$i+=1){ ?>&nbsp;&nbsp;<?php } ?>
                         <?php $__FOR_START_13767__=0;$__FOR_END_13767__=$rs['level'];for($i=$__FOR_START_13767__;$i < $__FOR_END_13767__;$i+=1){ ?>∟<?php } echo ($rs['catename']); ?>
                      </td>
                      <td class="text-center">
                         <?php if($rs['model'] == 0): ?><span class="text-red">单页</span><?php elseif($rs['model'] == 1): ?><span class="text-aqua">文章</span><?php else: ?><span class="text-green">图片</span><?php endif; ?>
                      </td>
                      <td class="text-center">
                         <?php if($rs['isnav'] == 0): ?><span class="text-red">隐藏</span><?php else: ?><span class="text-green">正常</span><?php endif; ?>
                      </td>
                      <td class="text-center">
                            <?php if($rs['issys'] == 0): ?><a class="btn btn-xs btn-info" href="<?php echo U('/Category/edit',array('id'=>$rs['id']));?>">
...
$(function(){
   $('.btn-sub-taxis').click(function(){
	  ...
	  $.ajax({
			 url: "<?php echo U('/Role/updatetaxis',array('tabel'=>'category'));?>", 
			 type: "POST",
